    ...
    <>
      {logos.map((p) => (
        <div key={p.src} className="group relative flex shrink-0 items-center">
          ...
          />
          {/* Name reveals underneath the logo on hover (band pauses on hover). */}
          <span
            role="tooltip"
            className="pointer-events-none absolute left-1/2 top-full z-10 mt-2 -translate-x-1/2 whitespace-nowrap rounded-full bg-ink-900/95 px-3 py-1.5 text-xs font-medium text-creamy-50 opacity-0 shadow-lg transition-opacity duration-200 group-hover:opacity-100"
          >
            {p.name}
          </span>
        </div>
